...
if __name__ == '__main__':
    # 命令行参数解析
    parser = argparse.ArgumentParser(description='FCD-GAN for Weakly Supervised Change Detection')
    
    # 数据路径参数
    parser.add_argument('--img_dir_x', type=str, required=True, 
                        help='时相 1 图像目录 (before)')
    parser.add_argument('--img_dir_y', type=str, required=True, 
                        help='时相 2 图像目录 (after)')
    ...
    # 输出路径参数
    parser.add_argument('--out_dir', type=str, default='output/Detection_WSS', 
                        help='输出目录 (相对路径，相对于项目根目录)')
    parser.add_argument('--out_gmodel_dir', type=str, default='output/GModel', 
                        help='生成器模型输出目录 (相对路径)')
    ...
    # 损失函数权重参数
    parser.add_argument('--perception_weight', type=float, default=0.5, 
                        help='感知损失权重')
    parser.add_argument('--ssim_weight', type=float, default=0, 
                        help='SSIM 损失权重')
    parser.add_argument('--g_weight', type=float, default=0.2, 
                        help='生成器损失权重')
    parser.add_argument('--l1_weight', type=float, default=1.6, 
                        help='L1 损失权重')
    parser.add_argument('--d_weight', type=float, default=1, 
                        help='判别器损失权重')
    parser.add_argument('--nc_weight', type=float, default=1.5, 
                        help='未变化样本变化图损失权重')
    
    # 其他参数
    parser.add_argument('--perception_layer', type=int, default=1, 
                        help='感知损失层数')
    parser.add_argument('--perception_per_band', action='store_true', 
                        help='是否按波段计算感知损失')
    parser.add_argument('--write_grey', action='store_true', default=True, 
                        help='是否输出灰度图 (变化概率图)')
    parser.add_argument('--write_color', action='store_true', default=True, 
                        help='是否输出彩色图 (TP/TN/FP/FN)')
    parser.add_argument('--model_g_reuse', action='store_true', default=True, 
                        help='是否复用已训练的生成器模型')
    parser.add_argument('--discriminator_continuous', action='store_true', default=True, 
                        help='判别器是否使用连续变化图')
    parser.add_argument('--tips', type=str, default='train', 
                        help='记录到 txt 文件的备注信息')
    ...
    # 设置输出路径 (相对路径，相对于项目根目录)
    if args.ext_name:
        out_dir_name = 'Detection_WSS{}'.format(args.ext_name)
    else:
        out_dir_name = 'Detection_WSS'
    
    OutDir = os.path.join(project_root, args.out_dir)
    OutGModelDir = os.path.join(project_root, args.out_gmodel_dir)
    
    # 确保输出目录存在
    if not os.path.exists(OutDir):
        os.makedirs(OutDir)
    if not os.path.exists(OutGModelDir):
        os.makedirs(OutGModelDir)
    
    if args.write_grey:
        OutDensityDir = OutDir + "_Density"
        if not os.path.exists(OutDensityDir):
            os.makedirs(OutDensityDir)

    # 设置数据路径
    ImgDirX = args.img_dir_x
    ImgDirY = args.img_dir_y
    RefDir = args.ref_dir
    LabelDir = args.label_dir
    
    # 设置训练参数
    init_num_epochs_G = args.init_epochs_g
    num_epochs = args.num_epochs
    learning_rate = args.lr
    unc_batch_size = args.unc_batch_size
    ...
    perception_perBand = args.perception_per_band
    perception_layer = args.perception_layer
    g_weight = args.g_weight
    l1_weight = args.l1_weight
    d_weight = args.d_weight
    nc_weight = args.nc_weight
    write_grey = args.write_grey
    write_color = args.write_color
    modelG_reuse = args.model_g_reuse
    discriminator_continuous = args.discriminator_continuous

    # TensorBoard 日志
    log_name = 'Building_WSSS{}'.format(args.ext_name) if args.ext_name else 'Building_WSSS'
